...
<body>
  ...
  <script type="module">
    ...
    const [categories, allProducts] = await Promise.all([
      ...
      dataService.getProductsByType('${device.type}')
    ]);
    ...
    if (replaceParam) {
      ...
      replaceBanner.innerHTML = \`
        <div style="display:flex;align-items:center;gap:var(--space-2);color:#92400e;font-size:var(--text-xs);font-weight:700">
          ...
          <div>
            <div>Changing Skin for ${device.name}</div>
    ...
    function renderFiltered() {
      ...
      const compatible = products.filter(p => p.compatibleDevices && p.compatibleDevices.includes(activeDeviceId));
...
devices.filter(d => d.type === 'phone').forEach(device => {
  ...
  console.log(`Generated: /phone-skins/${device.id}/`);
});

console.log('Done generating phone model slug pages!');
